...
import { 
  ...
  StatusBar, Platform, TouchableOpacity, Alert 
} from 'react-native';
...
export default function EditarMoneda() {
  ...
  // 🔹 Cargar moneda guardada al abrir
  useEffect(() => {
    const cargarMoneda = async () => {
      const guardada = await AsyncStorage.getItem("monedaPrincipal");
      if (guardada) {
        setMonedaActual(guardada);
        setMonedaSeleccionada(guardada);
      }
    };
    cargarMoneda();
  }, []);

  // 🔹 Guardar moneda + volver a configuración
  const handleGuardar = async () => {
    if (monedaSeleccionada === monedaActual) {
      Alert.alert("Sin cambios", "Ya has seleccionado esta moneda.");
      return;
    }

    await AsyncStorage.setItem("monedaPrincipal", monedaSeleccionada);
    setMonedaActual(monedaSeleccionada);

    Alert.alert("¡Éxito!", `Tu moneda ha sido actualizada a ${monedaSeleccionada}.`);

    setTimeout(() => {
      navigation.navigate("Configuracion"); // ← Aquí regresa
    }, 400);
  };

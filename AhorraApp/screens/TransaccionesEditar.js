...
export default function TransaccionesEditar () {

  const [categoria, setCategoria] = useState('Mascotas');
  const [monto, setMonto] = useState('-500');
  const [tipo, setTipo] = useState('Gasto');  
  const [dia, setDia] = useState('27');
  ...
  const handleGuardarCambios = () => {
    ...
    if (isNaN(montoNum)) {
      Alert.alert('Monto Inválido', 'El monto debe ser un número (ej: 500 o -300).');
      return;
    }
    if (montoNum === 0) {
    ...
    if (isNaN(diaNum) || diaNum < 1 || diaNum > 31) {
      Alert.alert('Día Inválido', 'Por favor, ingresa un día válido (entre 1 y 31).');
      return;
    }
